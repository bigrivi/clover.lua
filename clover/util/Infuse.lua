...
function MappingVo:init(prop,value,cl,singleton)
	self.prop = prop
	self.value = value
	self.cl = cl
...
function Injector:instantiate(targetClass,...)
	local args = {...}
	local real_args = getArgs(targetClass.init)
	table.remove(real_args,1)
	local new_args = {}
	for i=#args+1,#real_args do
		local vo = self:getMappingVo(real_args[i])
		local injectValue = self:getInjectedValue(vo)
		table.insert(args,injectValue)
	end 
	local instance = targetClass(unpack(args))
	return instance
end
